...
这里是创作、配图和草稿发布的配置步骤。首次体验请从仓库首页的离线演示开始。
...
安装到 `~/.codex/skills/wechat-ink-v2`，不会覆盖其他已安装技能。修改安装目录的 `wechat-ink.yaml`，填入自己的公众号、图片服务和固定出口配置。配置只保存在本机，不能提交到公开仓库。
...
- 默认 `academic-paper` 对齐常用字号、字体与图片间距；其他 14 种配色仍需逐项视觉验收。
- 配图接口、封面裁剪、草稿上传与回读、重复提交保护有离线测试。
- `newspic` 为实验入口；没有真实微信联调证据。跨平台同步、浏览器 Cookie 生图和自定义样式 JSON 尚未提供。
...
本目录新编写的代码和文档采用 MIT，见 LICENSE。运行依赖的许可见 THIRD_PARTY.md。
