...
这是微墨的技能工具包，包含写作规范、学术风主题、配图、排版、审校和微信草稿工具。没有分享者的账号、密钥、服务器配置、文章或历史图片。
...
外层 `install.py` 可复制技能到默认 `~/.codex/skills/wechat-ink`（如设置 CODEX_HOME 则跟随），并建立独立运行环境：

```sh
python3 install.py
```

Windows 原生命令通常为 `py install.py`。需要联网从 Python 包仓库安装依赖。安装器不会覆盖已有目录；若已安装同名技能，请先自行选择保留方案。可以用 `--target /你的/技能目录/wechat-ink` 指定目标，用 `--skip-deps` 只复制，稍后自行安装 requirements.txt。若中途依赖下载失败，按终端提示重试，不要重复覆盖安装。
...
安装器已从示例创建空白 `wechat-ink.yaml`。填写：
...
> 使用 wechat-ink 围绕“你选择的主题”研究并写一篇公众号文章。先核对原始来源，使用学术风排版，制作 1 张封面和 6 张正文图，交付文稿与本地预览，暂不创建微信草稿。
...
Gemini Web 的默认本地数据目录也改为微墨专用目录；如使用该可选后端，需要重新登录，或通过 GEMINI_WEB_DATA_DIR 显式指定自己原有的数据目录。
