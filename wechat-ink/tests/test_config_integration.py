...
def _prepare_isolated_skill_root(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    for filename in ("config.py", "generate_image.py", "ink_image_gen_core.ts"):
        shutil.copy(repo_root / "scripts" / filename, scripts_dir / filename)

    (skill_root / "wechat-ink.yaml").write_text(
        textwrap.dedent(
            """\
            default: main
            accounts:
    ...
    cwd_dir.mkdir()
    (cwd_dir / "wechat-ink.yaml").write_text(
        textwrap.dedent(
            """\
            default: main
            accounts:
    ...
    home_cfg_dir = home_dir / ".wechat-ink"
    home_cfg_dir.mkdir(parents=True)
    (home_cfg_dir / "wechat-ink.yaml").write_text(
        textwrap.dedent(
            """\
            default: main
            accounts:
...
def test_generate_image_print_command_uses_skill_root_yaml_only(tmp_path):
    ...
    assert "generator: ink-gemini-web" in result.stdout


@pytest.mark.skipif(shutil.which("bun") is None, reason="可选 Bun 后端未安装；Python 主流程不依赖 Bun")
def test_bun_load_env_uses_skill_root_yaml_only(tmp_path):
    """ink_image_gen_core.loadEnv() should ignore cwd/home YAML and export from its own skill root."""
    _, scripts_dir, cwd_dir = _prepare_isolated_skill_root(tmp_path)
    home_dir = tmp_path / "home"

    code = textwrap.dedent(
        """\
        import { loadEnv, detectProvider, getDefaultModel } from "./scripts/ink_image_gen_core.ts";
        ...
        console.log(JSON.stringify({
          ...
          generator: process.env.WECHAT_INK_IMAGE_GENERATOR,
        }));
        """
    )
    ...
    assert result.stdout.strip() == (
        ...
        '"model":"gemini-3-pro-image-preview","generator":"ink-gemini-web"}'
    )


@pytest.mark.skipif(shutil.which("bun") is None, reason="可选 Bun 后端未安装；Python 主流程不依赖 Bun")
def test_bun_load_env_prefers_canonical_yaml_over_ambient_provider_env(tmp_path):
    """ink_image_gen_core.loadEnv() should let canonical YAML override conflicting shell provider env."""
    skill_root = tmp_path / "skill"
    scripts_dir = skill_root / "scripts"
    scripts_dir.mkdir(parents=True)
    ...
    shutil.copy(repo_root / "scripts" / "ink_image_gen_core.ts", scripts_dir / "ink_image_gen_core.ts")
    ...
    code = textwrap.dedent(
        """\
        import { loadEnv, detectProvider } from "./scripts/ink_image_gen_core.ts";
...
def test_bun_load_env_does_not_mask_invalid_skill_root_yaml(tmp_path, yaml_text):
    """ink_image_gen_core.loadEnv() should fail on invalid or empty skill-root YAML even if shell provider env exists."""
    skill_root = tmp_path / "skill"
    scripts_dir = skill_root / "scripts"
    scripts_dir.mkdir(parents=True)
    ...
    (skill_root / "wechat-ink.yaml").write_text(yaml_text, encoding="utf-8")
