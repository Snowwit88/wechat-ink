...
def publish_from_markdown(
    ...
    try:
        ...
        print(f"  API连接失败：{e}")
        print("  请检查 wechat-ink.yaml 中的 app_id / app_secret 配置")
        sys.exit(1)
...
def publish_from_brief(
    ...
    if not images_dir.exists():
        raise FileNotFoundError(
            ...
            f"请先用 newspic_build.py + scripts/ink_image_gen.ts 生成贴图,保存为 {images_dir}/01.png 起。"
        )
...
示例：
  ...
  python publish.py --input article.md --cover cover.jpg --ai-score-threshold 55
    ...
    parser.add_argument("--account", help="指定公众号账号（对应 wechat-ink.yaml 中的账号名）")
    parser.add_argument(
        "--sync",
...
def _resolve_config(args):
    ...
    try:
        ...
        if args.sync_from_config:
            ...
            print("--sync-from-config 需要有效的 wechat-ink.yaml 配置。", file=sys.stderr)
            sys.exit(1)
