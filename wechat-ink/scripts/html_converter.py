...
REFERENCE_TEXT_SIZE = "14.5px"
REFERENCE_URL_SIZE = "12.5px"
REFERENCE_URL_STYLE = (
    f"font-size: {REFERENCE_URL_SIZE}; color: #6b7280; line-height: 1.45; "
    "letter-spacing: 0; word-break: break-all; overflow-wrap: anywhere;"
)
...
def convert_markdown_to_wechat_html(
    ...
    reference_list_counter = 0

    # ---- list_style 渲染辅助 ----
    def _num_token(n: int, compact=False) -> str:
        prefix = list_style.get("num_prefix", "")
        suffix = list_style.get("num_suffix", "")
        # 参考文献固定使用阿拉伯数字；括号、字体和间距仍沿用当前主题。
        fmt = "decimal" if compact else list_style.get("num_formatter", "decimal")
        container = list_style.get("num_container", "")
        if compact:
            container = f"{container}; font-size: {REFERENCE_TEXT_SIZE};"
    ...
    def flush_list():
        nonlocal in_list, list_items, list_type, reference_list_counter
        if in_list and list_items:
            tag = list_type or "ul"
            is_reference = reference_heading_level is not None
            li_style = styles["li"]
            list_container_style = styles[tag]
            if is_reference:
                # 参考资料里的来源与裸 URL 是一个条目，不沿用正文列表的大外边距。
                list_container_style = (
                    f"{list_container_style}; margin: 12px 0 16px; padding-left: 0;"
                )
                li_style = (
                    f"{li_style}; font-size: {REFERENCE_TEXT_SIZE}; "
                    "margin: 0 0 14px; padding-left: 0; line-height: 1.7;"
                )
            if tag == "ul":
                items_html = "\n".join(
                    f'<li style="{li_style}">{_bullet_token()}{item}</li>'
                    for item in list_items
                )
            else:
                start = reference_list_counter + 1 if is_reference else 1
                items_html = "\n".join(
                    f'<li style="{li_style}">{_num_token(n, is_reference)}{item}</li>'
                    for n, item in enumerate(list_items, start)
                )
                if is_reference:
                    reference_list_counter += len(list_items)
            html_parts.append(f'<{tag} style="{list_container_style}">{items_html}</{tag}>')
            list_items = []
            in_list = False
            list_type = None
    ...
    def process_inline(text, plain_reference_links=False):
        ...
        - 链接 [text](url) 的 url 若以 javascript: / data: / vbscript: 开头
          (忽略大小写和前导空白),直接退化为纯文本
        - “参考资料／参考文献”章节中的安全链接退化为“题名（裸 URL）”明文，
          不生成外部 <a>；正文其他章节的链接行为不变
        ...
        # 参考资料中的裸 URL 保持为可复制的明文，仅用 span 缩小字号并保证换行。
        # 排除紧跟在引号或等号后的 URL，避免误处理已生成标签的属性值。
        if plain_reference_links:
            text = re.sub(
                r'(?<!["\'=])https?://[^\s<>"）】》；，。]+',
                lambda m: f'<span style="{REFERENCE_URL_STYLE}">{m.group(0)}</span>',
                text,
            )
    ...
    for line in lines:
        ...
        if stripped.startswith(">"):
            ...
            blockquote_lines.append(process_inline(
                content, plain_reference_links=reference_heading_level is not None
            ))
            prev_was_h2 = False
            continue
        else:
        ...
        if h_match:
            ...
            if reference_heading_level is not None and level <= reference_heading_level:
                reference_heading_level = None
            if re.fullmatch(r'参考(?:资料|文献)', heading_text):
        ...
        if ul_match:
            if not in_list or list_type != "ul":
                ...
                in_list = True
                list_type = "ul"
            list_items.append(process_inline(
                ul_match.group(1), plain_reference_links=reference_heading_level is not None
            ))
            prev_was_h2 = False
            continue
        ...
        if ol_match:
            if not in_list or list_type != "ol":
                ...
                list_type = "ol"
            list_items.append(process_inline(
                ol_match.group(1), plain_reference_links=reference_heading_level is not None
            ))
            prev_was_h2 = False
            continue

        # 参考资料常用“来源说明 + 下一行裸 URL”。把 URL 接在同一个列表项里，
        # 避免渲染成带正文首行缩进和大段间距的独立段落。
        if (
            reference_heading_level is not None
            and in_list
            and list_items
            and re.fullmatch(r'https?://\S+', stripped)
        ):
            url_text = process_inline(stripped, plain_reference_links=True)
            list_items[-1] += f'<br><span style="display:block; margin-top: 2px;">{url_text}</span>'
            prev_was_h2 = False
            continue

        # 参考资料条目之间允许空行，但保持为同一个列表；遇到真正的正文时再 flush。
        if not stripped and reference_heading_level is not None and in_list:
            continue
        ...
        # 普通段落
        text = process_inline(
            stripped, plain_reference_links=reference_heading_level is not None
        )
