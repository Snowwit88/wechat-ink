...
仅支持 skill 根目录下的 wechat-ink.yaml。
...
UNIFIED_CONFIG_NAME = "wechat-ink.yaml"
DEFAULT_CODEX_BUILTIN_IMAGEGEN_FALLBACK = True
...
def _find_unified_yaml() -> Optional[Path]:
    """仅查找 skill 根目录下的 wechat-ink.yaml。"""
    path = _config_path()
...
def _load_unified_env() -> None:
    ...
    _set_env_if_present("WECHAT_INK_IMAGE_GENERATOR", image.get("generator"))
...
def load_env() -> None:
    """
    从 wechat-ink.yaml 加载辅助环境变量到 os.environ(不覆盖已有值)。
    """
    _load_unified_env()


def _load_config_yaml(yaml_path: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """加载 skill 根目录 wechat-ink.yaml。返回 None 表示找不到文件。"""
...
def get_codex_builtin_imagegen_fallback_enabled() -> bool:
    """读取 Codex 内置 ImageGen 兜底开关；未配置时默认开启。"""
    yaml_config = _load_config_yaml()
    if not yaml_config:
        return DEFAULT_CODEX_BUILTIN_IMAGEGEN_FALLBACK

    image_generation = yaml_config.get("image_generation") or {}
    if not isinstance(image_generation, dict):
        return DEFAULT_CODEX_BUILTIN_IMAGEGEN_FALLBACK

    value = image_generation.get(
        "codex_builtin_fallback", DEFAULT_CODEX_BUILTIN_IMAGEGEN_FALLBACK
    )
    ...
    if normalized in {"0", "false", "no", "off"}:
        return False
    raise ConfigError("image_generation.codex_builtin_fallback 必须是布尔值")
...
def get_config(account_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    优先级:
      ...
      3. wechat-ink.yaml 的 `default` 字段
    ...
    if not yaml_config or "accounts" not in yaml_config or not yaml_config["accounts"]:
        raise ConfigError(
            "未找到 wechat-ink.yaml 或文件为空。请参考 wechat-ink.yaml.example 创建配置。"
        )
    ...
    if account_name is None:
        ...
        raise ConfigError(
            f"未指定账号且 wechat-ink.yaml 里无 default。可用账号: {available}"
        )
...
def resolve_image_style(
    ...
    按优先级解析最终使用的配图风格:
      ...
      3. wechat-ink.yaml 对应账号的 image_style 字段
