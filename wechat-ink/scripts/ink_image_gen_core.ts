...
const CONFIG_PATH = path.join(SKILL_ROOT, "wechat-ink.yaml");
...
`.trim();
  ...
  if (result.exitCode !== 0) {
    ...
    throw new Error(stderr || "Failed to load wechat-ink.yaml via scripts/config.py");
  }
  ...
  if (!output) {
    if (!hasCanonicalConfigFile() && hasConfiguredProviderEnv()) return;
    throw new Error("wechat-ink.yaml did not export any image provider settings");
  }
  ...
  try {
    ...
    if (hasCanonicalConfig && !hasLoadedProviderEnv(loaded)) {
      throw new Error(
        "wechat-ink.yaml does not configure OPENAI_API_KEY or GEMINI_PROXY_API_KEY"
      );
    }
