...
@pytest.mark.parametrize("heading", ["参考资料", "参考文献"])
def test_reference_link_rendered_as_plain_url(theme, heading):
    """参考文献保留题名和裸 URL，但不生成会被微信清除的外部 <a>。"""
    md = (
        "正文 [说明文档](https://example.com/body)\n\n"
        f"## {heading}\n\n"
        "1. [研究报告](https://example.com/report?a=1&b=2)\n"
    )
    html = _convert(md, theme)

    assert html.count("<a ") == 1
    assert 'href="https://example.com/body"' in html
    assert "研究报告（<span" in html
    assert "font-size: 12.5px" in html
    assert "https://example.com/report?a=1&amp;b=2</span>）" in html
    assert 'href="https://example.com/report' not in html


def test_reference_text_and_url_use_smaller_sizes(theme):
    """参考条目略小于正文，裸 URL 再小一级，并允许长地址换行。"""
    md = "## 参考资料\n\n1. 研究报告；https://example.com/very-long-source\n"
    html = _convert(md, theme)

    assert 'font-size: 14.5px' in html
    assert 'font-size: 12.5px' in html
    assert 'word-break: break-all; overflow-wrap: anywhere;' in html
    assert '<a ' not in html


def test_reference_uses_arabic_numbers_without_changing_body_lists(theme):
    """参考文献用阿拉伯数字，正文有序列表仍遵循主题编号格式。"""
    styles, highlights, divider, list_style = theme
    roman_list_style = {
        **list_style,
        "num_prefix": "(",
        "num_suffix": ")",
        "num_formatter": "roman_lower",
    }
    md = "1. 正文条目\n\n## 参考资料\n\n1. 研究报告；https://example.com/report\n"
    html = convert_markdown_to_wechat_html(
        md, styles, highlights, divider, roman_list_style
    )
    body_html, reference_html = html.split(">参考资料</h2>", 1)

    assert ">(i)</span>" in body_html
    assert ">(1)</span>" in reference_html
    assert ">(i)</span>" not in reference_html


def test_reference_numbers_continue_across_bare_url_paragraphs(theme):
    """来源与下一行裸 URL 分段时，后续参考条目不应重新从 1 编号。"""
    md = (
        "## 参考资料\n\n"
        "1. 第一份报告；\n"
    ...
    html = _convert(md, theme)
    _, reference_html = html.split(">参考资料</h2>", 1)

    number_tokens = re.findall(
        r"<li[^>]*><span[^>]*>([^<]+)</span>", reference_html
    )
    assert number_tokens == ["1", "2"]
    assert reference_html.count("<ol") == 1
    assert "第一份报告；<br><span" in reference_html
    assert "text-indent: 2em" not in reference_html.split("</ol>", 1)[0]


def test_link_after_reference_section_kept(theme):
    """离开参考文献章节后，正文链接恢复原有 <a> 行为。"""
    md = (
        "## 参考资料\n\n"
        "1. [研究报告](https://example.com/report)\n\n"
        "## 附录\n\n"
        "查看 [附录文档](https://example.com/appendix)\n"
    )
    html = _convert(md, theme)

    assert 'href="https://example.com/report"' not in html
    assert 'href="https://example.com/appendix"' in html
