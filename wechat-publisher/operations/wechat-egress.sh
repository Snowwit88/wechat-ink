...
load_config() {
  ...
  required_vars="WECHAT_EGRESS_HOST WECHAT_EGRESS_USER WECHAT_EGRESS_SSH_PORT WECHAT_EGRESS_LOCAL_PORT WECHAT_EXPECTED_EGRESS_IP WECHAT_EGRESS_AUTH_MODE WECHAT_PUBLISHER_HOME WECHAT_PUBLISHER_PYTHON"
...
ensure_publisher() {
  [ -d "$WECHAT_PUBLISHER_HOME/scripts" ] || die "找不到安装版发布工具：$WECHAT_PUBLISHER_HOME"
  [ -x "$WECHAT_PUBLISHER_PYTHON" ] || die "找不到发布工具 Python：$WECHAT_PUBLISHER_PYTHON"
  "$WECHAT_PUBLISHER_PYTHON" -c 'import requests, socks, yaml' >/dev/null 2>&1 || die "发布环境缺少 requests、PySocks 或 PyYAML。"
}
...
token_check() {
  ...
  ensure_publisher
  export_proxy_env
  cd "$WECHAT_PUBLISHER_HOME/scripts"
  "$WECHAT_PUBLISHER_PYTHON" -c 'from wechat_api import get_access_token; token = get_access_token(force_refresh=True); print("微信 token 验证成功（长度 %d，不显示内容）" % len(token))'
}
...
publish_draft() {
  ...
  set +e
  PYTHONUNBUFFERED=1 "$WECHAT_PUBLISHER_PYTHON" publish.py "$@" 2>&1 \
