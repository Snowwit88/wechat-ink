...
[▶ 打开在线演示](https://snowwit88.github.io/wechat-ink/) · [快速体验](#快速体验) · [开始创作](#开始创作)

![微墨：文稿到手机排版的样稿展示](docs/assets/preview.svg)
...
[**立即打开在线演示，查看完整手机样稿 →**](https://snowwit88.github.io/wechat-ink/)

演示附带七张测试图；浏览网页无需公众号、密钥或本地安装，也不会产生服务费用。

> 离线演示使用预置文章和程序绘制的测试图，不是现场 AI 生成。真实生图和微信草稿流程已实现，但尚未完成真实服务验收。

## 快速体验

### 在线体验

打开[微墨演示网站](https://snowwit88.github.io/wechat-ink/)，向下滚动右侧手机样稿，就能查看文章、配图、图注和参考资料。

### 离线查看（可选）

[下载项目 ZIP](https://github.com/Snowwit88/wechat-ink/archive/refs/heads/main.zip) 后，解压并双击 `demo.html`。已有 Python 也可以运行：

```sh
python3 demo.py
```

离线演示只用 Python 标准库，不安装依赖、不读取凭证、不调用任何 API。Windows 可用 `python demo.py`。
...
需要 Python 3.10+。默认安装为独立技能 `wechat-ink-v2`，不会覆盖其他已安装技能。安装后，在支持本地技能的 AI 客户端中使用下面这段话：
...
离线样稿、排版和安装流程已通过测试。真实生图、微信草稿和客户端最终效果仍待验收；[查看验证记录](VALIDATION.md)。
...
当前版本代码采用 [MIT](LICENSE)，依赖说明见 [THIRD_PARTY.md](THIRD_PARTY.md)。
